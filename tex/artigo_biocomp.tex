...
\footertext{\textit{Ciências da Computação} (2025) - Estudo Científico}
\setcounter{page}{1}
...
\author{%
	Kauan Elias Schneider Fank\textsuperscript{1}\thanks{Corresponding author: \href{mailto:kauan.fank@ufsc.br}{kauan.fank@ufsc.br}}
}
...
Uma Rede de Petri é um grafo bipartido $N = (P, T, F, W, M_0)$ onde $P$ são lugares (espécies moleculares), $T$ são transições (reações), $F$ são arcos direcionados, $W$ são pesos dos arcos, e $M_0$ é a marcação inicial. A extensão estocástica (SPN) associa a cada transição $t_j$ uma taxa de disparo $\lambda_j$, transformando a rede em um processo de Markov. A função de propensidade é:
...
Ferramentas existentes como COPASI~\cite{karlebach2008} e CellDesigner oferecem funcionalidades similares, porém com interfaces gráficas complexas e curvas de aprendizado íngremes. Nosso pipeline, implementado em Python com módulos independentes, prioriza:
\begin{itemize}
...
Extensões planejadas incluem: análise de sensibilidade paramétrica para identificar parâmetros críticos; suporte a SBML Level 3 com extensões (comp, fbc); interface web interativa; e integração com ferramentas de otimização para design de circuitos \textit{de novo}.
